...
> **Sprint 5.3 (sin cambio de STATE_VERSION):** logo topbar 44px; topbar `position:fixed`; penalizaciones: fix pool agotado + `visibilitychange` listener + `bootSystem` fallback + botón debug en Settings; eliminado tab Menú (Reset movido a Settings → Zona de Peligro); tarjetas modo rediseñadas con imagen full-bleed, gradiente cinemático, estrellas de dificultad, colores de selección por modo; dirección visual Genshin Impact registrada en docs.
